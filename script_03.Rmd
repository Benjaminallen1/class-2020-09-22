...
#Gather all the possible data
#Limit our search to pertinent information 
#Understand that students drop and add into new classes 
#Group duplicate classes together
#Drop data that is not applicable
Understand the mentality of students once classes begin 
To see if the classes are enjoyable/manageable to students based on if they drop it or not
Include a survey to students who dropped classes to find out why they dropped 

After we have a model, Temperance suggests we should:

Recognize the numbers are hard to use to interpret students' mentalities 
Understand that there are external factors to a student dropping a class 
...
```{r}
sep_old
left_join(sep_new,sep_old, by= "id",
          suffic=c("_old", "_new"))
?join
```
